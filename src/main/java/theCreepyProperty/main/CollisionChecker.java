...
//    }
    ...
    public void checkCollisionmyTest1(Player player, ArrayList<LevelData.LevelDataWall> wallsListe, ArrayList<LevelData.LevelDataItem> items, MapCreate mapCreate) {
        for (int i = 0; i < scene.getMapCreate().getWallList().size() ; i++) {
            boolean collision = checkCollision1(player, player.getSolidPlayerAria(), scene.getMapCreate().getWallList().get(i).getRWall());
...
//            switch (player.direction) {
//                case "up":
//                    if (mapCreate.getWall().getPlayer_block_collision() && collision) {
//                        player.collision_on = true;
//                        return;
//                    }
//                    break;
...
//            }
            ...
            //System.out.println("sitze: " + scene.getMapCreate().getWallList().size());
            //System.out.println("rWall: " + scene.getMapCreate().getWallList().get(i).getRWall());
            //todo entfernen
            if (collision == true) {
                System.out.println("Coliding: " + collision);
            }else if (player.collision_on == true) {
                System.out.println("Player colision: true");
            }
        }
    }

    public void checkCollision(Player player, ArrayList<LevelData.LevelDataWall> wallsListe, ArrayList<LevelData.LevelDataItem> items, MapCreate mapCreate) {
        for (int i = 0; i < scene.getMapCreate().getWallList().size(); i++) {
            Rectangle wallRectangle = scene.getMapCreate().getWallList().get(i).getRWall();

            // Spieler-Kopie mit hypothetischer neuer Position erstellen
            Rectangle futurePlayer = new Rectangle(player.getSolidPlayerAria().getX(), player.getSolidPlayerAria().getY(),
                    player.getSolidPlayerAria().getWidth(), player.getSolidPlayerAria().getHeight());

            // Bewegung simulieren
            switch (player.direction) {
                case "up":
                    futurePlayer.setY(futurePlayer.getY() - player.speed);
                    break;
                case "down":
                    futurePlayer.setY(futurePlayer.getY() + player.speed);
                    break;
                case "left":
                    futurePlayer.setX(futurePlayer.getX() - player.speed);
                    break;
                case "right":
                    futurePlayer.setX(futurePlayer.getX() + player.speed);
                    break;
            ...
            // Prüfen, ob der simulierte Spieler kollidiert
            if (futurePlayer.intersects(wallRectangle.getBoundsInLocal())) {
                player.collision_on = true;
                return; // Stoppe die Prüfung, falls eine Wand getroffen wurde
            }
        }

        player.collision_on = false; // Keine Kollision -> Bewegung erlauben
    }


    private boolean checkCollision1(Player player, Rectangle rPlayer, Rectangle rectangle) {
        if (rPlayer.intersects(rectangle.getBoundsInLocal())) {
